...
class OutterProduct(nn.Module):
    ...
    def forward(self, inputs):#inputs len=26 [32, 1, 8]
        embed_list = inputs
        row = []
        col = []
        num_inputs = len(embed_list)
        ...
        p = torch.cat([embed_list[idx] for idx in row], dim=1)  # batch num_pairs k
        q = torch.cat([embed_list[idx] for idx in col], dim=1)
        #row len=325,col len=325,p[32, 325, 8],q[32, 325, 8]
        # -------------------------
        if self.kernel_type == 'mat':
            p.unsqueeze_(dim=1)#p [32, 1, 325, 8]
            # k     k* pair* k
            # batch * pair
            #kp [32, 325]
            kp = torch.sum(torch.mul(torch.transpose(torch.sum(torch.mul(p, self.kernel), dim=-1), 2, 1), q), dim=-1)#mul [32, 8, 325, 8],sum [32, 8, 325],tran [32, 325, 8]
        else:
            # 1 * pair * (k or 1)
